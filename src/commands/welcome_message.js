...
module.exports = {
	...
	async execute(interaction) {
      ...
      \nPara selecionar basta reagir a essa mensagem de acordo com a experiência escolhida. E claro, você pode escolher mais de uma!`)
				.setFields(
        ...
        \nLevantamento de copo — 🍺` },
			...
			if (await storage.getItem('welcome_message')) {
				await storage.updateItem('welcome_message', message.id);
			}
			else {
				await storage.setItem('welcome_message', message.id);
			}
